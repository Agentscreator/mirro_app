...
export default function AIPromptInput({ method, onGenerate, onBack, initialInput = "" }: AIPromptInputProps) {
  ...
  const getPlaceholder = () => {
    switch (method) {
      case "generate":
        return "e.g., 'Birthday party tomorrow at 7 PM at Central Park' or 'Team meeting next Friday morning in conference room'"
      case "paste":
        return "Paste your event details here... (AI will automatically extract dates, times, and locations)"
      case "import":
    ...
    <div>
      ...
      <div className="mb-10 text-center">
        ...
        <p className="text-text-secondary font-normal">
          AI will automatically extract dates, times, and locations from your text
        </p>
      </div>

      <div className="space-y-6">
        {/* AI Extraction Info */}
        <div className="glass-card rounded-2xl p-4 bg-gradient-to-r from-taupe-50 to-cream-50 border border-taupe-200">
          <div className="flex items-start space-x-3">
            <div className="w-8 h-8 bg-taupe-200 rounded-lg flex items-center justify-center flex-shrink-0">
              <svg className="w-4 h-4 text-taupe-600" fill="currentColor" viewBox="0 0 20 20">
                <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clipRule="evenodd" />
              </svg>
            </div>
            <div className="flex-1">
              <h4 className="text-sm font-medium text-taupe-700 mb-1">Smart Extraction</h4>
              <p className="text-xs text-taupe-600 leading-relaxed mb-2">
                AI automatically detects: dates (tomorrow, Dec 15, next Friday), times (7 PM, morning, 19:00), and locations (Central Park, downtown, my office)
              </p>
              {method === 'generate' && (
                <div className="text-xs text-taupe-500">
                  <strong>Try:</strong> "Birthday party tomorrow at 7 PM at Central Park" or "Team meeting next Friday morning in conference room A"
                </div>
              )}
            </div>
          </div>
